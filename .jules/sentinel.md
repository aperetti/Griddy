...
## 2024-05-22 - Command Injection & Path Traversal in Admin Upload Route
**Vulnerability:** The `/install` route in `admin-console/admin-backend/src/features/extensions/routes.ts` allowed command injection via shell metacharacters in uploaded filenames due to the use of `child_process.exec()` with string interpolation. Furthermore, the uploaded filename was directly appended to `/tmp` without validation, enabling path traversal attacks.
**Learning:** Shell string interpolation for system commands is notoriously vulnerable when any part of the command incorporates user input. Always use safe execution methods that pass arguments directly (like `execFile` or `spawn`) bypassing the shell interpreter, and rigorously sanitize file names using `path.basename()` before mapping them to the filesystem.
**Prevention:** Use `child_process.execFile()` with explicitly parameterized arguments instead of `exec()` for shell commands. Use `path.basename(filename)` to extract solely the file name, dropping any provided path prefix or traversal sequences (`../`).
