...
## 2026-05-26 - Frontend Client-Side Authentication Bypass
**Vulnerability:** The `DisplayRulesManager` frontend component implemented administrative authentication using a hardcoded string comparison (`username === 'admin' && password === 'admin'`) before setting the `adminAuth` token in `localStorage`.
**Learning:** Client-side authentication checks are inherently insecure, as the credentials and logic are exposed in the client bundle and can be trivially bypassed or reverse-engineered. Authentication must always be verified by the backend.
**Prevention:** Rather than trusting client-side comparisons, the frontend should encode the provided credentials into a Basic Auth token, store it temporarily, and make an authenticated API request (e.g., `fetchDisplayConfigs()`). If the request succeeds, the credentials are valid; if it returns a 401, the token must be cleared.
