...
## 2026-06-03 - Missing Authentication on Admin Endpoints & Password Parsing Flaw
**Vulnerability:** The admin console backend API endpoints (e.g., configurations, users, plugins, telemetry) were missing authentication, allowing unauthenticated users to perform sensitive administrative actions. Additionally, the manual parsing of HTTP Basic Auth credentials via `split(':')` incorrectly truncated passwords containing a colon, creating a functional bug for valid credentials.
**Learning:** All administrative endpoints must be explicitly protected by authentication middleware (e.g., a Fastify preHandler hook). When implementing custom Basic Auth validation, robust string splitting must be used to handle arbitrary characters (like colons) in user passwords, and time-constant comparison functions (like `crypto.timingSafeEqual`) must be used to prevent timing attacks when comparing password hashes.
**Prevention:** Wrap sensitive route registrations in an authenticated context block (e.g., `fastify.register` scoped with the auth hook). For string parsing of credentials, use index-based splitting (e.g., `indexOf(':')`) rather than generic `split(':')` destructuring.
