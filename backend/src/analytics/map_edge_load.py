...
class MapEdgeLoadUseCase:
    ...
    _VALID_AGGS = {"mean": "AVG", "min": "MIN", "max": "MAX", "median": "MEDIAN"}
    ...
    def _get_parquet_range_files(self, start_time: str, end_time: str) -> List[str]:
        ...
        if not relevant_files:
            logger.info(
                "No unified month-stamped files matched range, falling back to all parquet files."
            )
            relevant_files = [str(p) for p in Path(self.parquet_dir).glob("*.parquet")]

        logger.info(
            f"Heatmap range {start_time} to {end_time} selected {len(relevant_files)} files: {covered_months}"
        )
    ...
    def estimate(
        ...
        if not relevant_files:
            return {
                "estimated_rows": 0,
                "error": "No load data available in range.",
    ...
    def execute(
        ...
        if not relevant_files:
            return {
                "error": "No load data available in the requested range.",
                "start_time": start_time,
                "end_time": end_time,
            }
        ...
        try:
            ...
            node_loads = {
                row[0]: float(row[1]) for row in results if row[1] is not None
            }
            ...
            if not hasattr(self.graph_engine, "graph") or not hasattr(
                self.graph_engine, "flow_depth"
            ):
            ...
            sorted_nodes = sorted(
                flow_depth.keys(), key=lambda n: flow_depth[n], reverse=True
            )
            edge_total_loads = {}
            ...
            for node_v in sorted_nodes:
                ...
                for u, v, data in incoming_edges:
                    ...
                    if not eid:
                        continue
                    edge_total_loads[eid] = (
                        edge_total_loads.get(eid, 0.0) + load_per_edge
                    )
