...
async def get_voltage_estimate(
    ...
    logger.debug("Voltage estimate requested for %d nodes", len(node_ids))
...
async def get_consumption_estimate(
    ...
    ensure_graph_built()
    node_ids = node_id.split(",")
    logger.debug("Consumption estimate requested for %d nodes", len(node_ids))
...
async def get_map_voltage_estimate(
    ...
    ensure_graph_built()
    logger.debug("Map voltage estimate requested (agg=%s, node_id=%s)", agg, node_id)
...
async def get_map_voltage(
    ...
    ensure_graph_built()
    logger.info("Generating map voltage summary (agg=%s, root_node=%s)", agg, node_id)
    result = await run_in_threadpool(map_voltage_uc.execute, start_time, end_time, agg, node_id)
    if "error" in result:
        logger.error("Map voltage summary failed: %s", result["error"])
        raise HTTPException(status_code=400, detail=result["error"])
    return result
