...
def test_topological_load_aggregation(mock_graph_engine, temp_parquet_dir, tmp_path):
    ...
    uc = MapEdgeLoadUseCase(mock_graph_engine, db_path, temp_parquet_dir)
    
    # Test file pruning logic
    files = uc._get_parquet_range_files("2026-04-01T00:00:00", "2026-04-01T23:59:59")
    assert len(files) == 1
    assert "2026_04" in files[0]
    assert "2026_01" not in files[0]

    # Test multi-month pruning
    multi_files = uc._get_parquet_range_files("2026-01-01T00:00:00", "2026-04-15T23:59:59")
...
def test_topological_load_aggregation_no_data(mock_graph_engine, tmp_path):
    ...
    uc = MapEdgeLoadUseCase(mock_graph_engine, db_path, str(empty_dir))
